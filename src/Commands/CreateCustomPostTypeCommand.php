...
class CreateCustomPostTypeCommand extends Command
{
    ...
    private function generatePostType(): array
    {
        ...
        return [
            ...
            'hierarchical' => $this->io->confirm('Is it hierarchical ?', false) ? 'true' : 'false',
            ...
            'show_in_admin_bar' => $this->io->confirm('Show in admin bar ?', true) ? 'true' : 'false',
            'menu_position' => $this->io->ask('Menu Position', '5'),
            'can_export' => $this->io->confirm('Can export ?', true) ? 'true' : 'false',
            'has_archive' => $this->io->confirm('Has archive ?', true) ? 'true' : 'false',
            'exclude_from_search' => $this->io->confirm('Exclude from search ?', false) ? 'true' : 'false',
